...
function Receipt() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-4">
        <div className="mx-auto max-w-4xl rounded-[22px] bg-white p-8 text-center shadow-md">
          <p className="text-base font-semibold text-slate-700">Loading receipt...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.10),_transparent_32%),linear-gradient(to_bottom,_#f8fafc,_#eef2ff)] px-3 py-4 print:bg-white">
          ...
          <div className="bg-gradient-to-r from-sky-600 via-cyan-500 to-indigo-500 px-4 py-5 text-white sm:px-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <h1 className="mt-3 text-2xl font-bold">Medicine Receipt</h1>
                <p className="mt-1 text-sm text-cyan-50/90">Receipt for order {order.id}</p>
              </div>
          ...
          <div className="p-4 sm:p-5">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl bg-slate-50 p-3">
                ...
                <p className="mt-1 text-sm font-bold text-slate-800">{order.id}</p>
              </div>
              <div className="rounded-2xl bg-slate-50 p-3">
              ...
              <div className="rounded-2xl bg-slate-50 p-3">
                ...
                <div className="mt-1 flex flex-col gap-1">
                  ...
                  <span
                    className={`inline-flex w-fit items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${getPaymentStatusColor(
                      order.payment_status
                    )}`}
                  >
                    {getPaymentStatusIcon(order.payment_status)}
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
              <div className="rounded-[22px] border border-slate-100 p-4">
                <p className="mb-3 text-sm font-bold text-slate-800">Customer Details</p>
              ...
              <div className="rounded-[22px] border border-slate-100 p-4">
                <p className="mb-3 text-sm font-bold text-slate-800">Payment Summary</p>
                <div className="space-y-2.5">
                  ...
                  <div className="flex items-center justify-between rounded-2xl bg-orange-50 px-4 py-3">
                    ...
                    <span className="font-bold text-orange-600">₹{commission.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-2xl bg-green-50 px-4 py-3">
                    <span className="text-sm text-slate-600">Pharmacy Payout</span>
                    <span className="font-bold text-green-700">₹{pharmacyPayout.toFixed(2)}</span>
                  </div>
            ...
            <div className="mt-4 rounded-[22px] border border-slate-100 p-4">
              <p className="mb-3 text-sm font-bold text-slate-800">Ordered Medicines</p>
                    ...
                    >
                      ...
                      <div className="text-left sm:text-right">
                        ...
                        <p className="text-xs text-orange-600">
                          Commission: {Number(item.commission_percent || 0)}%
                        </p>
                      </div>
                    </div>
                  ))}
